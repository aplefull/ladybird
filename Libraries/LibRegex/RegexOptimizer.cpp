...
void Optimizer::append_character_class(ByteCode& target, Vector<CompareTypeAndValuePair>&& pairs)
{
    ...
    bool has_string_set = any_of(pairs, [](auto& pair) {
        return pair.type == CharacterCompareType::StringSet;
    });

    target.empend(static_cast<ByteCodeValueType>(has_string_set ? OpCodeId::CompareStringSet : OpCodeId::Compare));
